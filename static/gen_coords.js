...
// Mapping brightness between 0 and 10
...
// Is in function (standard JS func doesn't work for 2d arrays)
...
// Calculates adjacent pixels of same brightness
...
function update_bar(size, i) {
    // Progress bar (does not fully work)
...
export async function get_bodies(obj_data) {
    ...
    for (let row = 0; row < obj_data.length; row++) {
        for (let col = 0; col < obj_data[0].length; col++) {
            ...
            // Only continue if it is not in array already
            if (!is_in(checked, [row, col])) {
                ...
                // Updating bar occasionally
                if (i % 100 === 0 || i === size) {
                    update_bar(size, i);
                    await sleep(20);
                }

                // Keep going while there are more pixels to be checked per body
    ...
    // Returning each body as a collection of pixels
    return bodies;
}
